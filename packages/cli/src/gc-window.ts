/**
 * Flush scheduling and GC-window estimation for the direct-upload flow
 * (issue #70).
 *
...
/**
 * FWSS `addPieces` batch cap. synapse-core >= 1.x exports this as
 * `SIZE_CONSTANTS.MAX_ADD_PIECES_BATCH_SIZE`; the pinned 0.5.x does not, so
 * it lives here until the SDK is bumped.
 */
export const MAX_ADD_PIECES_BATCH = 40
...
/**
 * Margin derived from confirmations observed earlier in the same run, per
 * issue #70 — a constant cannot cover a congested chain. Doubling the worst
 * observed commit duration leaves room for one full retry; the floor covers
 * the first flush of a run, before any observation exists.
 */
 ...
 * the caller should retain locally and upload in a burst instead (rvagg:
 * "don't dribble this in").
 */
