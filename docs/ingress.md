...
### Networks that block outbound port 7844
...
`--ingress cloudflared` detects the missing registration and restarts the
tunnel once with `--protocol http2` (TCP) on its own, logging the switch.

If both transports fail (the network blocks port 7844 entirely), run the
tunnel however your network allows and hand the URL over:
